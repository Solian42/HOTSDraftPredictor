...
from PegasosPredictor import PegasosPredictor
from KNNPredictor import KNNPredictor, DistanceKNNPredictor
from AdaBoost import AdaBoostPredictor
    ...
    with open(filename) as reader:
        for line in reader:
            ...
            try:
                ...
                raise ValueError("Unable to convert " + label_string + " to integer in " + get_args().data)
...
def train(instances):
    ...
    if algorithm == "pegasos":
        predictor = PegasosPredictor(args.pegasos_lambda, args.online_training_iterations)
        predictor.train(instances)
        return predictor
    elif algorithm == "knn":
        predictor = KNNPredictor(args.knn)
        predictor.train(instances)
        return predictor
    elif algorithm == "distance_knn":
        predictor = DistanceKNNPredictor(args.knn)
        predictor.train(instances)
        return predictor
    elif algorithm == "adaboost":
        predictor = AdaBoostPredictor(args.num_boosting_iterations)
        predictor.train(instances)
        return predictor
    else:
        return None
